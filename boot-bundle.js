...
(function () {
  ...
  function hydrate_(payloads) {
    ...
    for (var i = 0; i < types.length; i++) {
      ...
      /* Tell the gate this type was fetched, or its own loader sees no fetch history, goes
         ...
         the bundle actually replace that request rather than precede it. The built time is 0 for
         everything: an honest "unknown" that leaves the age chip on its fetch-time fallback, and
         a value noteBuiltAt() ignores, so OLT keeps the real stamp applyOltPayload() just took
         from its payload. */
      if (typeof fetchGate !== 'undefined' && fetchGate && typeof fetchGate.noteHydrated === 'function') {
        fetchGate.noteHydrated(type, 0);
      }
  ...
  /**
   * Ask for the whole opening at once.
   *
   * NEVER REJECTS. The resolved summary is:
   *   { used, hydrated[], refused[], missing[], reason }
   * `used` is false whenever nothing could be hydrated, and the caller's job is then to load
   * modules the way it did before this file existed.
   *
    ...
    return withTimeout_(fetchWithRetry(BASE_API_URL + '?action=bundle', 1, 800), BUNDLE_TIMEOUT_MS)
      .then(function (res) {
        if (!res || res.ok !== true || !res.bundle || typeof res.bundle !== 'object') {
          return { used: false, hydrated: [], refused: [], missing: [],
